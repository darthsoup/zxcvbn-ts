...
function build_ranked_dict(ordered_list) {
  const result = {}
...
const matching = {
  ...
  extend(lst, lst2) { return lst.push.apply(lst, lst2) },
  translate(string, chr_map) { return (Array.from(string.split('')).map(chr => chr_map[chr] || chr)).join('') },
  mod(n, m) { return ((n % m) + m) % m }, // mod impl that works for negative numbers
  sorted(matches) {
  ...
  omnimatch(password) {
    ...
    for (const matcher of Array.from(matchers)) {
  ...
  dictionary_match(password, _ranked_dictionaries) {
    ...
    if (_ranked_dictionaries == null) { _ranked_dictionaries = RANKED_DICTIONARIES }
    const matches = []
  ...
  enumerate_l33t_subs(table) {
    ...
    const helper = function (keys) {
      ...
      const next_subs = []
      for (const l33t_chr of Array.from(table[first_key])) {
        for (const sub of Array.from(subs)) {
          let dup_l33t_index = -1
          for (let i = 0, end = sub.length, asc = end >= 0; asc ? i < end : i > end; asc ? i++ : i--) {
            if (sub[i][0] === l33t_chr) {
              dup_l33t_index = i
              break
            }
  ...
  l33t_match(password, _ranked_dictionaries, _l33t_table) {
    let token
    if (_ranked_dictionaries == null) { _ranked_dictionaries = RANKED_DICTIONARIES }
    if (_l33t_table == null) { _l33t_table = L33T_TABLE }
    const matches = []
    for (const sub of Array.from(this.enumerate_l33t_subs(this.relevant_l33t_subtable(password, _l33t_table)))) {
      if (this.empty(sub)) { break } // corner case: password has no relevant subs.
      const subbed_password = this.translate(password, sub)
      for (const match of Array.from(this.dictionary_match(subbed_password, _ranked_dictionaries))) {
  ...
  spatial_match(password, _graphs) {
    if (_graphs == null) { _graphs = GRAPHS }
    const matches = []
  ...
  sequence_match(password) {
    ...
    const update = (i, j, delta) => {
      if (((j - i) > 1) || (Math.abs(delta) === 1)) {
        let middle
        if ((middle = Math.abs(delta)) > 0 && middle <= this.MAX_DELTA) {
    ...
    for (let k = 1, end = password.length, asc = end >= 1; asc ? k < end : k > end; asc ? k++ : k--) {
  ...
  regex_match(password, _regexen) {
    if (_regexen == null) { _regexen = REGEXEN }
    const matches = []
...
`)
    ...
    for (i = 0, end = password.length - 4, asc = end >= 0; asc ? i <= end : i >= end; asc ? i++ : i--) {
      var asc1, end1, start
      for (start = i + 3, j = start, end1 = i + 7, asc1 = start <= end1; asc1 ? j <= end1 : j >= end1; asc1 ? j++ : j--) {
        if (j >= password.length) { break }
        token = password.slice(i, +j + 1 || undefined)
        if (!maybe_date_no_separator.exec(token)) { continue }
        const candidates = []
        for (const [k, l] of Array.from(DATE_SPLITS[token.length])) {
          dmy = this.map_ints_to_dmy([
            Number.parseInt(token.slice(0, k)),
            Number.parseInt(token.slice(k, l)),
            Number.parseInt(token.slice(l)),
          ])
        ...
        for (const candidate of Array.from(candidates.slice(1))) {
    ...
    for (i = 0, end2 = password.length - 6, asc2 = end2 >= 0; asc2 ? i <= end2 : i >= end2; asc2 ? i++ : i--) {
      ...
      for (start1 = i + 5, j = start1, end3 = i + 9, asc3 = start1 <= end3; asc3 ? j <= end3 : j >= end3; asc3 ? j++ : j--) {
        ...
        if (rx_match == null) { continue }
        dmy = this.map_ints_to_dmy([
          Number.parseInt(rx_match[1]),
          Number.parseInt(rx_match[3]),
          Number.parseInt(rx_match[4]),
        ])
    ...
    return this.sorted(matches.filter((match) => {
      ...
      for (const other_match of Array.from(matches)) {
  ...
  map_ints_to_dmy(ints) {
    ...
    let dm, rest, y
    if ((ints[1] > 31) || (ints[1] <= 0)) { return }
    let over_12 = 0
    let over_31 = 0
    let under_1 = 0
    for (const int of Array.from(ints)) {
      if ((int > 99 && int < DATE_MIN_YEAR) || (int > DATE_MAX_YEAR)) { return }
      if (int > 31) { over_31 += 1 }
      if (int > 12) { over_12 += 1 }
      if (int <= 0) { under_1 += 1 }
    }
    if ((over_31 >= 2) || (over_12 === 3) || (under_1 >= 2)) { return }
