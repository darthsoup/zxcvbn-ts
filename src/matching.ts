...
const build_ranked_dict = function(ordered_list) {
  ...
  for (var word of Array.from(ordered_list)) {
    result[word] = i;
    i += 1;
  }
  return result;
...
const matching = {
  ...
  omnimatch(password) {
    ...
    for (var matcher of Array.from(matchers)) {
  ...
  relevant_l33t_subtable(password, table) {
    ...
    for (var chr of Array.from(password.split(''))) {
      password_chars[chr] = true;
    }
    const subtable = {};
    for (var letter in table) {
      var subs = table[letter];
      var relevant_subs = (Array.from(subs).filter((sub) => sub in password_chars));
  ...
  enumerate_l33t_subs(table) {
    ...
    var helper = function(keys) {
      ...
      const next_subs = [];
      for (var l33t_chr of Array.from(table[first_key])) {
        for (var sub of Array.from(subs)) {
    ...
    for (var sub of Array.from(subs)) {
      var sub_dict = {};
      for (var [l33t_chr, chr] of Array.from(sub)) {
  ...
  l33t_match(password, _ranked_dictionaries, _l33t_table) {
    ...
    const matches = [];
    for (var sub of Array.from(this.enumerate_l33t_subs(this.relevant_l33t_subtable(password, _l33t_table)))) {
      if (this.empty(sub)) { break; } // corner case: password has no relevant subs.
      var subbed_password = this.translate(password, sub);
      for (var match of Array.from(this.dictionary_match(subbed_password, _ranked_dictionaries))) {
    ...
    while (i < (password.length - 1)) {
      ...
      while (true) {
        ...
        if (j < password.length) {
          ...
          for (var adj of Array.from(adjacents)) {
            cur_direction += 1;
  ...
  repeat_match(password) {
    ...
    while (lastIndex < password.length) {
      ...
      var [i, j] = Array.from([match.index, (match.index + match[0].length) - 1]);
...
`);
    ...
    for (i = 0, end = password.length - 4, asc = 0 <= end; asc ? i <= end : i >= end; asc ? i++ : i--) {
      ...
      for (start = i + 3, j = start, end1 = i + 7, asc1 = start <= end1; asc1 ? j <= end1 : j >= end1; asc1 ? j++ : j--) {
        ...
        var candidates = [];
        for (var [k,l] of Array.from(DATE_SPLITS[token.length])) {
        ...
        var metric = candidate => Math.abs(candidate.year - scoring.REFERENCE_YEAR);
        var min_distance = metric(candidates[0]);
        for (var candidate of Array.from(candidates.slice(1))) {
    ...
    return this.sorted(matches.filter(function(match) {
      ...
      for (var other_match of Array.from(matches)) {
  ...
  map_ints_to_dmy(ints) {
    ...
    let over_31 = 0;
    let under_1 = 0;
    for (var int of Array.from(ints)) {
      if ((99 < int && int < DATE_MIN_YEAR) || (int > DATE_MAX_YEAR)) { return; }
      if (int > 31) { over_31 += 1; }
      if (int > 12) { over_12 += 1; }
      if (int <= 0) { under_1 += 1; }
    }
  ...
  map_ints_to_dm(ints) {
    for (var [d, m] of [ints, ints.slice().reverse()]) {
