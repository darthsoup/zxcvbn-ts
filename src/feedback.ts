...
var feedback = {
  ...
  get_feedback(score, sequence) {
    // starting feedback
    if (sequence.length === 0) { return this.default_feedback; }
    ...
    for (var match of Array.from(sequence.slice(1))) {
      if (match.token.length > longest_match.token.length) { longest_match = match; }
    }
