...
const scoring = {
  ...
  most_guessable_match_sequence(password, matches, _exclude_additive) {
    ...
    for (var lst of Array.from(matches_by_j)) {
  ...
  uppercase_variations(match) {
    ...
    for (var regex of [this.START_UPPER, this.END_UPPER, this.ALL_UPPER]) {
  ...
  l33t_variations(match) {
    ...
    for (var subbed in match.sub) {
      ...
      if ((S === 0) || (U === 0)) {
        ...
        var p = Math.min(U, S);
        var possibilities = 0;
        for (var i = 1, end = p, asc = 1 <= end; asc ? i <= end : i >= end; asc ? i++ : i--) { possibilities += this.nCk(U + S, i); }
