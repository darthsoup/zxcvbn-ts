...
const scoring = {
  nCk(n, k) {
    ...
    if (k === 0) { return 1 }
    let r = 1
    for (let d = 1, end = k, asc = end >= 1; asc ? d <= end : d >= end; asc ? d++ : d--) {
      r *= n
      r /= d
      n -= 1
    }
  ...
  factorial(n) {
    ...
    if (n < 2) { return 1 }
    let f = 1
    for (let i = 2, end = n, asc = end >= 2; asc ? i <= end : i >= end; asc ? i++ : i--) { f *= i }
    return f
  },
  ...
  most_guessable_match_sequence(password, matches, _exclude_additive) {
    ...
    const matches_by_j = ((() => {
      let asc, end
      const result = []
      for (_ = 0, end = n, asc = end >= 0; asc ? _ < end : _ > end; asc ? _++ : _--) {
        result.push([])
      }
      return result
    })())
    ...
    const optimal = {
      ...
      m: (((() => {
        let asc1, end1
        const result1 = []
        for (_ = 0, end1 = n, asc1 = end1 >= 0; asc1 ? _ < end1 : _ > end1; asc1 ? _++ : _--) {
          result1.push({})
        }
        return result1
      })())),
      ...
      pi: (((() => {
        let asc2, end2
        const result2 = []
        for (_ = 0, end2 = n, asc2 = end2 >= 0; asc2 ? _ < end2 : _ > end2; asc2 ? _++ : _--) {
          result2.push({})
        }
        return result2
      })())),
      ...
      g: (((() => {
        let asc3, end3
        const result3 = []
        for (_ = 0, end3 = n, asc3 = end3 >= 0; asc3 ? _ < end3 : _ > end3; asc3 ? _++ : _--) {
          result3.push({})
        }
        return result3
      })())),
    ...
    const bruteforce_update = (k) => {
      ...
      return (() => {
        const result4 = []
        for (var i = 1, end4 = k, asc4 = end4 >= 1; asc4 ? i <= end4 : i >= end4; asc4 ? i++ : i--) {
    ...
    for (k = 0, end4 = n, asc4 = end4 >= 0; asc4 ? k < end4 : k > end4; asc4 ? k++ : k--) {
    ...
    if (match.regex_name in char_class_bases) {
      ...
      switch (match.regex_name) {
        ...
        // if year is close to REFERENCE_YEAR, estimate a year space of MIN_YEAR_SPACE.
          var year_space = Math.abs(Number.parseInt(match.regex_match[0]) - this.REFERENCE_YEAR)
  ...
  spatial_guesses(match) {
    ...
    for (i = 2, end = L, asc = end >= 2; asc ? i <= end : i >= end; asc ? i++ : i--) {
      const possible_turns = Math.min(t, i - 1)
      for (let j = 1, end1 = possible_turns, asc1 = end1 >= 1; asc1 ? j <= end1 : j >= end1; asc1 ? j++ : j--) {
    ...
    if (match.shifted_count) {
      ...
      if ((S === 0) || (U === 0)) {
        ...
        for (i = 1, end2 = Math.min(S, U), asc2 = end2 >= 1; asc2 ? i <= end2 : i >= end2; asc2 ? i++ : i--) { shifted_variations += this.nCk(S + U, i) }
  ...
  uppercase_variations(match) {
    ...
    let variations = 0
    for (let i = 1, end = Math.min(U, L), asc = end >= 1; asc ? i <= end : i >= end; asc ? i++ : i--) { variations += this.nCk(U + L, i) }
    return variations
  },
  ...
  l33t_variations(match) {
    ...
    for (var subbed in match.sub) {
      ...
      if ((S === 0) || (U === 0)) {
        ...
        let possibilities = 0
        for (let i = 1, end = p, asc = end >= 1; asc ? i <= end : i >= end; asc ? i++ : i--) { possibilities += this.nCk(U + S, i) }
